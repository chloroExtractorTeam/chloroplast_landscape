...
automized?
*** chloroExtractor
The chloroExtractor *REF CHLORO* is the tool which was programmed by the chloroExtractorteam *GITHUB LINK*  from the CCTB *LINK*. ChloroExtractor uses a kmer based approach to detect the chloroplast
...
Jellyfish *JELLYFISH LINK*. To assamble the kmers SPAdes is used. The main parts of the tool are programmed in perl. 
